...
function createTelegramPhotoMessage(chatId = "123456") {
	return {
		message_id: Math.floor(Math.random() * 100000),
		chat: { id: Number(chatId) },
...
function createTelegramCallback(data: string, chatId = "123456") {
	return {
...
describe("User Signup Flow", () => {
	...
	test("new user with valid invite code is created via Telegram message", async () => {
		...
		const welcomeMsg = sentMessages.find(
			(m) =>
				m.chatId === chatId &&
				m.text?.includes("Welcome to the Dunnes Voucher Bot"),
		);
