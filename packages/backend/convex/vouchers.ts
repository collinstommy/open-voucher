...
export const requestVoucher = internalMutation({
	...
	handler: async (ctx, { userId, type }) => {
		...
		// Schedule reminder message for next day at 10am
		const tomorrow = dayjs().add(1, "day");
		const tomorrowAt10am = tomorrow.hour(10);
		const delayMs = tomorrowAt10am.diff(dayjs());

		await ctx.scheduler.runAfter(
			delayMs,
			internal.telegram.sendVoucherUploadReminderAction,
			{
