...
- Upload voucher → OCR extracts type/expiry → earn coins
- Claim voucher → receive image → spend coins
- Report "already used" → get replacement or refund
- Basic coin economy
...
3. Bot validates code (passed via /start parameter) → creates user → sends welcome message

**Returning users:**
1. User sends voucher photo to Telegram bot
2. Bot uploads image to Convex storage
3. Gemini OCR extracts voucher type (€5/€10/€20) and expiry date
...
| Scale | Monthly Cost |
|-------|--------------|
| 500 vouchers/day | ~€1/month (within Gemini free tier) |
| 1000 vouchers/day | ~€4/month |
| 2000 vouchers/day | ~€10/month |
...
| Action | Coins | Notes |
|--------|-------|-------|
| Signup bonus | +10 | One-time on first message |
| Upload €5 voucher | +15 | Highest reward (smallest voucher) |
| Upload €10 voucher | +10 | Medium reward |
| Upload €20 voucher | +5 | Lowest reward (largest voucher) |
| Claim €5 voucher | -15 | Highest cost |
| Claim €10 voucher | -10 | Medium cost |
| Claim €20 voucher | -5 | Lowest cost |
...
Smaller vouchers (€5) cost more coins because:
1. They're easier to use (only €25 spend required)
...
export const handleTelegramMessage = internalAction({
  ...
  handler: async (ctx, { message }) => {
    ...
    if (!user) {
      ...
      if (text.startsWith("/start")) {
        ...
        if (code) {
           ...
           if (result.valid) {
             ...
             await sendTelegramMessage(chatId, `🎉 Welcome to Dunnes Voucher Bot!\n\nYou've been given ${newUser.coins} coins to start.`);
             return;
           } else {
      ...
      await sendTelegramMessage(chatId, "👋 Welcome! You need an invite link to join.");
      return;
    }
      ...
      await sendTelegramMessage(chatId, `Commands:\n📸 Send photo to upload\n💳 "claim 5/10/20"\n💰 /balance`);
       ...
       if (type && VOUCHER_TYPES.includes(type as any)) {
         try {
           ...
           await sendTelegramPhoto(chatId, imageUrl!, `Here is your €${type} voucher!`);
         } catch (e: any) {
...
1. Go to "Data" → "inviteCodes" table
...
| claim 5 | Claim €5 voucher |
| claim 10 | Claim €10 voucher |
| claim 20 | Claim €20 voucher |
